...
class Camera:
    """ Camera class storing intrinsic and extrinsic parameters along with image data. """
    ...
    def __init__(self,
                 ...
                 cx=-1, cy=-1,
                 fl_x=-1, fl_y=-1,
                 depth=None, resolution=None,
                 image_path=None, meta_only=False,
                 pts_depth=None, sky_mask=None):
        """ Initialize the Camera object with given parameters. """
        self.uid = uid
        ...
        if cx > 0:
            self.projection_matrix = \
                getProjectionMatrixCenterShift(
                    self.znear, self.zfar, cx, cy,
                    fl_x, fl_y, self.image_width, self.image_height
                ).transpose(0, 1)
        else:
            self.projection_matrix = \
                getProjectionMatrix(
    ...
    def get_rays(self):
        ...
        return (
                self.camera_center[None, None], directions
                / torch.norm(directions, dim=-1, keepdim=True)
        )

    def cuda(self):
        """ Move all tensor attributes of the Camera to the specified CUDA device. """
    ...
    def get_world_directions(self):
        """ Compute world space ray directions for each pixel in the image. """
        ...
        directions = torch.stack(
            [
                (u - self.image_width / 2 + 0.5 + du) / focal_x,
        ...
        directions = (
            self.c2w[:3, :3] @ directions.reshape(3, -1)
        ).reshape(3, self.image_height, self.image_width)
        return directions


class MiniCam:
    """ A minimal camera class storing essential parameters. """
    def __init__(self,
                 width, height,
                 fovy, fovx,
                 znear, zfar,
