...
class GaussianModel:
    """Class representing a Gaussian Model for 3D/4D Gaussian Splatting."""
    ...
    def __init__(self, sh_degree: int,
        ...
        """ Initialize Gaussian Model with given parameters. """
    ...
    def setup_functions(self):
        """ Setup activation functions for the Gaussian Model. """
        def build_covariance_from_scaling_rotation(scaling,
                                                   scaling_modifier,
                                                   rotation):
            """ Build covariance matrix from scaling and rotation for 3D Gaussians. """
            L = build_scaling_rotation(scaling_modifier * scaling, rotation)
            actual_covariance = L.transpose(1, 2) @ L
            symm = strip_symmetric(actual_covariance)
            return symm

        def build_covariance_from_scaling_rotation_4d(scaling,
                                                      scaling_modifier,
                                                      rotation_l,
                                                      rotation_r,
                                                      dt=0.0,
                                                      return_covt=False):
            """ Build covariance matrix from scaling and rotation for 4D Gaussians. """
            ...
            if dt.shape[1] > 1:
                mean_offset = (
                    cov_12.squeeze(-1) / cov_t.squeeze(-1)
                )[:, None, :] * dt[..., None]
    ...
    def capture(self):
        """ Capture the current state of the Gaussian Model. """
    ...
    def restore(self, model_args, training_args):
        """ Restore the Gaussian Model from saved state. """
        if self.gaussian_dim == 3:
            (self.active_sh_degree,
             self._xyz,
             self._features_dc,
        ...
        if not (self.indexed and '_scaling' in self.vq_attributes):
            ...
            if self.quantization and '_scaling' in self.qa_attributes:
                return self.scaling_activation(
                    self.scaling_qa(
                        self.vq_scaling_model.get_codes_from_indices(
                            self.rvq_indices_scaling
                        ).sum(dim=0).squeeze(0)
                    )
                )
            else:
    ...
    def get_scaling_t(self):
        """ Get scaling for 4D Gaussians along time dimension. """
        return self.scaling_activation(
            self._scaling_t if not self.hybrid else self.scaling_t_hybrid
        )

    @property
    def scaling_t_hybrid(self):
        """ Get scaling for 4D Gaussians in hybrid mode. """
    ...
    def rotation_r_hybrid(self):
        """ Get rotation for 4D Gaussians in hybrid mode. """
        ...
        if not (self.indexed and '_rotation' in self.vq_attributes):
            ...
            if self.quantization and '_rotation' in self.qa_attributes:
                return self.rotation_activation(
                    self.rotation_qa(
                        self.vq_rotation_model.get_codes_from_indices(
                            self.rvq_indices_rotation
                        ).sum(dim=0).squeeze(0)
                    )
                )
            else:
    ...
    def get_features(self):
        if not (self.indexed and '_features_dc' in self.vq_attributes):
            ...
            features_dc = self.vq_features_dc_model.get_codes_from_indices(
                self.rvq_indices_features_dc
            ).sum(dim=0).squeeze(0).unsqueeze(1).contiguous()
        if not (self.indexed and '_features_rest' in self.vq_attributes):
            features_rest = self._features_rest
        else:
            features_rest = self.vq_features_rest_model.get_codes_from_indices(
                self.rvq_indices_features_rest
            ).sum(dim=0).squeeze(0).view(
                self._features_rest.shape[0], -1, 3
            ).contiguous()
    ...
    def get_max_sh_channels(self):
        """ Get maximum number of spherical harmonic channels. """
    ...
    def get_cov_t(self, scaling_modifier=1):
        """ Get current covariance along time dimension for 4D Gaussians. """
        if self.rot_4d:
            L = build_scaling_rotation_4d(
    ...
    def get_marginal_t(self, timestamp, scaling_modifier=1):  # Standard
        """ Get marginal distribution along time dimension for 4D Gaussians. """
    ...
    def get_covariance(self, scaling_modifier=1):
        """ Get current covariance for 3D Gaussians. """
        return self.covariance_activation(
            self.get_scaling, scaling_modifier,
            self._rotation
        )

    def get_current_covariance_and_mean_offset(self,
                                               scaling_modifier=1,
                                               timestamp=0.0,
                                               return_covt=False):
        """ Get current covariance and mean offset for 4D Gaussians. """
        return self.covariance_activation(
    ...
    def create_from_pcd(self, pcd: BasicPointCloud,
        ...
        if self.gaussian_dim == 4:
            ...
            dist_t = torch.zeros_like(
                fused_times, device="cuda"
            ) + (
                    self.time_duration[1] - self.time_duration[0]
                ) / time_scale_factor
            ...
            if self.rot_4d:
                rots_r = torch.zeros(
                    (fused_point_cloud.shape[0], 4), device="cuda"
                )
                rots_r[:, 0] = 1
    ...
    def create_from_pth(self, path, spatial_lr_scale):
        """ Create Gaussian Model from a .pth file. """
    ...
    def training_setup(self, training_args):
        """ Setup the training optimizer and related parameters. """
    ...
    def reset_opacity(self):
        """ Reset opacity values to a minimum threshold. """
    ...
    def _prune_optimizer(self, mask):
        """ Prune optimizer parameters based on a given mask. """
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
    ...
    def prune_points(self, mask):
        """ Prune points from the Gaussian Model based on a given mask. """
        ...
        if self.indexed:
            if '_scaling' in self.vq_attributes:
                self.rvq_indices_scaling = \
                    self.rvq_indices_scaling[:, valid_points_mask]
            if '_rotation' in self.vq_attributes:
                self.rvq_indices_rotation = \
                    self.rvq_indices_rotation[:, valid_points_mask]
                self.rvq_indices_rotation_r = \
                    self.rvq_indices_rotation_r[:, valid_points_mask]
            if '_features_dc' in self.vq_attributes:
                self.rvq_indices_features_dc = \
                    self.rvq_indices_features_dc[:, valid_points_mask]
            if '_features_rest' in self.vq_attributes:
                self.rvq_indices_features_rest = \
                    self.rvq_indices_features_rest[:, valid_points_mask]

    def cat_tensors_to_optimizer(self, tensors_dict):
        """ Concatenate new tensors to the optimizer parameters. """
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
    ...
    def densify_and_split(self,
        ...
        if not self.rot_4d:
            ...
            if self.gaussian_dim == 4:
                ...
                new_scaling_t = self.scaling_inverse_activation(
                    ...
                    / ((0.8*N) if self.split_t else 1.0)
                )
        else:
            ...
            rots = build_rotation_4d(
                ...
                (
                    ...
                    if not self.hybrid
                    else self.rotation_r_hybrid[selected_pts_mask]
                )
            ).repeat(N, 1, 1)
            new_xyzt = torch.bmm(
                rots,
                samples.unsqueeze(-1)
            ).squeeze(-1) + self.get_xyzt[selected_pts_mask].repeat(N, 1)
        ...
        if self.hybrid:
            ...
            self.hybrid_mask = torch.cat(
                [self.hybrid_mask, new_hybrid_mask],
                dim=0
            )
    ...
    def densify_and_prune(self,
                          ...
                          max_grad_t=None,
                          prune_only=False):
        """ Densify and prune points based on gradient and opacity conditions. """
        if not prune_only:
            ...
            self.densify_and_clone(
                grads, max_grad, extent, grads_t, max_grad_t
            )
    ...
    def add_densification_stats(self,
                                ...
                                update_filter,
                                avg_t_grad=None):
        """ Add densification statistics based on gradients. """
    ...
    def add_densification_stats_grad(self,
                                     viewspace_point_grad,
                                     update_filter,
                                     avg_t_grad=None):
        """ Add densification statistics based on pre-computed gradients. """
        self.xyz_gradient_accum[update_filter] += \
    ...
    def post_quant(self, param, prune=False):
        """ Post-process quantization of parameters. """
    ...
    def huffman_encode(self, param):
        """ Huffman encode a parameter tensor and return its size in MB. """
    ...
    def vector_quantization(self, training_args, finetuning_lr_scale=0.1):
        """ Setup vector quantization for model parameters. """
        ...
        other_params = [
            {
                'params': [self._xyz],
                'lr': (
                        training_args.position_lr_final
                        * self.spatial_lr_scale
                        * finetuning_lr_scale
                ),
                "name": "xyz"
        ...
        if self.vq_attributes:
            if '_features_rest' in self.vq_attributes:
                ...
                self.rvq_bit_features_rest = math.log2(
                    self.rvq_size_features_rest
                )
                rest_features_rest_params = self._features_rest.flatten(1)
                self.vq_features_rest_model = ResidualVQ(
                        dim=rest_features_rest_params.shape[1],
                        codebook_size=self.rvq_size_features_rest,
                        num_quantizers=self.rvq_num_features_rest, 
                        commitment_weight=0.,
                        kmeans_init=True,
                        kmeans_iters=1,
                        ema_update=False,
                ...
                for _ in tqdm(range(self.rvq_iter_features_rest - 1)):
                    _, _, _ = self.vq_features_rest_model(
                        rest_features_rest_params.unsqueeze(0).detach()
                    )
                _, self.rvq_indices_features_rest, _ = \
                    self.vq_features_rest_model(
                        rest_features_rest_params.unsqueeze(0)
                    )
            ...
            if '_features_dc' in self.vq_attributes:
                ...
                self.vq_features_dc_model = ResidualVQ(
                    dim=3, codebook_size=self.rvq_size_features_dc,
                    num_quantizers=self.rvq_num_features_dc,
                    commitment_weight=0., kmeans_init=True,
                    kmeans_iters=1, ema_update=False,
                    learnable_codebook=True,
                    in_place_codebook_optimizer=lambda *args,
                    **kwargs: torch.optim.Adam(*args, **kwargs, lr=0.0001)
                ).cuda()
                for _ in tqdm(range(self.rvq_iter_features_dc - 1)):
                    _, _, _ = self.vq_features_dc_model(
                        base_features_dc_params.squeeze(1).unsqueeze(0)
                    )
                _, self.rvq_indices_features_dc, _ = \
                    self.vq_features_dc_model(
                        base_features_dc_params.squeeze(1).unsqueeze(0)
                    )
                codebook_params.append({
                    'params': [p for p in self.vq_features_dc_model.parameters()],
                    'lr': training_args.feature_lr * finetuning_lr_scale,
                    "name": "vq_features_dc"
                })
            else:
                other_params.append({
            ...
            if '_scaling' in self.vq_attributes:
                ...
                self.vq_scaling_model = ResidualVQ(
                    dim=3, codebook_size=self.rvq_size_scaling,
                    num_quantizers=self.rvq_num_scaling,
                    commitment_weight=0., kmeans_init=True,
                    kmeans_iters=1, ema_update=False,
                    learnable_codebook=True,
                    in_place_codebook_optimizer=lambda *args,
                    **kwargs: torch.optim.Adam(*args, **kwargs, lr=0.0004)
                ).cuda()

                for _ in tqdm(range(self.rvq_iter_scaling - 1)):
                    _, _, _ = self.vq_scaling_model(scaling_params.unsqueeze(0))
                _, self.rvq_indices_scaling, _ = \
                    self.vq_scaling_model(
                        scaling_params.unsqueeze(0)
                    )  # 1, N, 6
                codebook_params.append({
                    'params': [
                        p
                        for p in self.vq_scaling_model.parameters()
                    ],
                    'lr': training_args.scaling_lr * finetuning_lr_scale,
                    "name": "vq_scaling"
                })
            else:
                other_params.append({
            ...
            if '_rotation' in self.vq_attributes:
                ...
                rotation_params = torch.cat(
                    [self.get_rotation, self.get_rotation_r],
                    dim=0
                )
                self.vq_rotation_model = ResidualVQ(
                    dim=4, codebook_size=self.rvq_size_rotation,
                    num_quantizers=self.rvq_num_rotation,
                    commitment_weight=0., kmeans_init=True,
                    kmeans_iters=1, ema_update=False, 
                    learnable_codebook=True, 
                    in_place_codebook_optimizer=lambda *args,
                ...
                codebook_params.append({
                    'params': [p for p in self.vq_rotation_model.parameters()],
                    'lr': training_args.rotation_lr * finetuning_lr_scale,
                    "name": "vq_rotation"
                })
            else:
                other_params.append({
        ...
        if codebook_params:
            self.optimizer_codebook = torch.optim.Adam(
                codebook_params, lr=0.0, eps=1e-15
            )
        else:
            self.optimizer_codebook = None
        self.optimizer_others = torch.optim.Adam(
            other_params, lr=0.0, eps=1e-15
        )

    def fake_quantization(self):
        """ Setup fake quantization for model parameters. """
    ...
    def compute_storage(self, encode=False):
        ...
        if self.indexed and '_scaling' in self.vq_attributes:
            scale_indices_mb = self.huffman_encode(
                self.rvq_indices_scaling
            ) if encode else (
                self._xyz.shape[0]
                * self.rvq_bit_scaling
                * self.rvq_num_scaling
                / 8 / 10**6
            )
            scale_codebook_mb = self.huffman_encode(
                self.post_quant(
                    self.vq_scaling_model.codebooks.view(
                        -1,
                        self.vq_scaling_model.codebooks.shape[-1])
                )[1]
            ) if (
                    self.quantization 
                    and '_scaling' in self.qa_attributes
                    and encode
            ) else (
                    2 ** self.rvq_bit_scaling
                    * self.rvq_num_scaling
                    * 4
                    * scaling_precision
                    / 8 / 10**6
        ...
        if self.indexed and '_rotation' in self.vq_attributes:
            rotation_indices_mb = 2 * self.huffman_encode(
                self.rvq_indices_rotation
            ) if encode else (
                2 * self._xyz.shape[0]
                * self.rvq_bit_rotation
                * self.rvq_num_rotation
                / 8 / 10**6
            )
            rotation_codebook_mb = self.huffman_encode(
                self.post_quant(
                    self.vq_rotation_model.codebooks.view(
                        -1,
                        self.vq_rotation_model.codebooks.shape[-1]
                    )
                )[1]
            ) if (
                    self.quantization
                ...
                2**self.rvq_bit_rotation
                * self.rvq_num_rotation*8
                * rotation_precision
                / 8 / 10**6
            )
        ...
        if self.indexed and '_features_dc' in self.vq_attributes:
            dc_indices_mb = self.huffman_encode(
                self.rvq_indices_features_dc
            ) if encode else (
                2 * self._xyz.shape[0]
                * self.rvq_bit_features_dc
                * self.rvq_num_features_dc/8/10**6
            )
            rest_indices_mb = self.huffman_encode(
                self.rvq_indices_features_rest
            ) if encode else (
                2 * self._xyz.shape[0]
                * self.rvq_bit_features_rest
                * self.rvq_num_features_rest/8/10**6
            )
            dc_codebook_mb = self.huffman_encode(
                self.post_quant(
                    self.vq_features_dc_model.codebooks.view(
                        -1,
                        self.vq_features_dc_model.codebooks.shape[-1]
                    )
                )[1]
            ) if (
                    self.quantization
                    and '_features_dc' in self.qa_attributes
                    and encode
            ) else (
                2**self.rvq_bit_features_dc
                * self.rvq_num_features_dc
                * 8 * color_precision
                / 8 / 10**6
            )
            rest_codebook_mb = self.huffman_encode(
                self.post_quant(
                    self.vq_features_rest_model.codebooks.view(
                        -1,
                        self.vq_features_rest_model.codebooks.shape[-1]
                    )
                )[1]
            ) if (
                    self.quantization
                    and '_features_rest' in self.qa_attributes
                    and encode
            ) else (
                2**self.rvq_bit_features_rest
                * self.rvq_num_features_rest*8
                * color_precision/8
                / 10**6
            )
