...
class Scene:
    """ Scene class to handle loading and saving of scene data. """
    ...
    def __init__(self,
                 ...
                 num_pts_ratio=1.0,
                 time_duration=None):
        """ Initialize the Scene object by loading scene data. """
        ...
        if not self.loaded_iter:
            ...
            with open(
                    os.path.join(self.model_path, "cameras.json"),
                    'w'
                 ) as file:
        ...
        if args.loaded_pth:
            self.gaussians.create_from_pth(
                args.loaded_pth, self.cameras_extent
            )
        else:
            if self.loaded_iter:
    ...
    def getTrainCameras(self, scale=1.0):
        """ Get train cameras dataset. """
        return CameraDataset(
            self.train_cameras[scale].copy(),
            self.white_background
        )

    def getTestCameras(self, scale=1.0):
        """ Get test cameras dataset. """
        return CameraDataset(
            self.test_cameras[scale].copy(),
            self.white_background
        )
